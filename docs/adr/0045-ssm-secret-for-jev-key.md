...
- `infra/main.tf` creates an `aws_ssm_parameter` SecureString named
  `/<name-prefix>/typesafe-api-key` on real AWS only (`local.use_cdn`;
  LocalStack stays keyless with no SSM at all), seeded with a `CHANGE-ME`
  placeholder and `lifecycle.ignore_changes = [value]` so later applies
  never revert the real secret. Encryption uses the default `aws/ssm`
  key: no customer-managed key, no extra cost.
...
- The execution role gets `ssm:GetParameter` on exactly that parameter
  ARN plus `kms:Decrypt` on exactly the `alias/aws/ssm` key. The bootstrap
  deploy role gains the parameter/tag/KMS actions scoped to the project
  prefix and that same key alias, so `apply` can manage the parameter
...
- Fresh production runs detection-keyless (placeholder key fails Jev
  auth, which falls back to `"unknown"`) until the operator sets the real
  value — a quieter version of ADR-0037's misconfiguration risk, mitigated
  by the `typesafe_api_key_ssm_parameter` output, the placeholder wording,
  and the `infra/README.md` runbook.
